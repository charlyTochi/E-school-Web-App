...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <title>{{ __('E-School Management') }}</title>
    ...
    <link href="{{ asset('material') }}/custom.css" rel="stylesheet"/>
    </head>
    <body class="{{ $class ?? '' }}">
        @auth()
        ...
        $(document).ready(function(){
          $.get(url, function(response){
            ...
            //     Prices.push(data.created_at);
            // });
            var ctx = document.getElementById("canvas").getContext('2d');
                var myChart = new Chart(ctx, {
                  type: 'line',
                  data: {
                      labels:['Users', 'Parents', 'Teachers', 'Students'],
                      datasets: [{
                          label: 'Users',
                          data: [response['Users'], response['Parents'], response['Teachers'], response['Students']],
                          borderWidth: 2
                      }]
                  },
                  options: {
